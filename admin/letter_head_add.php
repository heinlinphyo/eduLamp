	...
	if(isset($_POST['save'])){
		$maxsize=1097152;
                    if($_FILES['image']['name']){
                    	...
                    	//check logo//
                    	$get_check=mysqli_query($conn,"SELECT * FROM letter_head order by id desc limit 1 ");
						...
						move_uploaded_file($image['tmp_name'],"letter_head/".$new);
						$insert_logo=mysqli_query($conn,"INSERT INTO letter_head (letter_image) VALUES ('$new') ");
...
<script>
	  $(document).ready(function() {
		swal.fire({
  			...
  			if (result.value) {
    			window.location = "../letter_head.php";
  			}
	});
});
</script>